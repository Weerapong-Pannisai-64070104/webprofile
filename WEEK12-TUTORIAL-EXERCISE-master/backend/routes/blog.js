...
router.get("/blogs/:id", function (req, res, next) {
  ...
  const promise2 = pool.query("SELECT comments.*, users.first_name, users.last_name  FROM users right join comments on (users.id = comments.comment_by_id) WHERE blog_id=?", [
    req.params.id,
  ]);
