...
const commentOwner = async (req, res, next) => {
    console.log("ooooooooo"+ req.params.commentId)
    if (req.user.role === 'admin') {
        return next()
    }
    const [[com]] = await pool.query('SELECT * FROM comments WHERE id=?', [req.params.commentId])
    if (com.comment_by_id !== req.user.id) {
...
router.post('/:blogId/comments', isLoggedIn, async function (req, res, next) {
    ...
    try {
        ...
        const [rows2, fields2] = await conn.query(
            'SELECT comments.*, users.first_name, users.last_name  FROM users right join comments on (users.id = comments.comment_by_id) WHERE comments.id = ?',
            [rows1.insertId]
        )
        await conn.commit()
        console.log(rows2[0])
