...
const blogsSchema = Joi.object({
  ...
  if(value.end_date && !value.start_date){
    throw new Joi.ValidationError("fill  start_date pls")
  }
  if(value.end_date < value.start_date){
    throw new Joi.ValidationError("end_date must be greater than start_date")
  }
  return value
})
...
router.post(
  ...
  async function (req, res, next) {
    if (req.method == "POST") {
      ...
      let pathArray = [];
      console.log(file, !file , file.length === 0, !!file)
      if (file.length === 0) {
      ...
      try {
        await blogsSchema.validateAsync(req.body, { abortEarly: false })
      } catch (err) {
        console.log("validate errer")
      ...
      try {
        console.log(reference, start_date, end_date)
        let results = await conn.query(
          "INSERT INTO blogs(title, content, status, pinned, `like`, reference, create_date, start_date, end_date) VALUES(?, ?, ?, ?, 0, ? ,CURRENT_TIMESTAMP, ?, ?);",
