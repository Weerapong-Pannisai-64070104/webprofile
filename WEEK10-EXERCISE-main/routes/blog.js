...
router.get("/blogs/:id", async function (req, res, next) {
  ...
  try {
    const promise1 =  conn.query("SELECT * FROM blogs WHERE id=?", [
      req.params.id,
    ]);
    
    const promise2 =  conn.query("SELECT * FROM comments left outer join images on comments.id = images.comment_id WHERE comments.blog_id=? order by comment_date", [
      req.params.id,
    ]);
    const promise3 =  conn.query("SELECT * FROM images WHERE blog_id=? and comment_id is null", [
      req.params.id,
    ]);
